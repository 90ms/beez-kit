...
> 현재 상태: 프로젝트 기반 구성 단계입니다. 아래 모듈의 기본 골격만 준비되어 있으며 공개 API는 아직 구현되지 않았습니다.

## 제공 라이브러리

| 분류 | 라이브러리 | 아티팩트 | 상태 | 상세 스펙 |
| --- | --- | --- | --- | --- |
| 툴킷 | 쓰로틀링 | `io.github.beez:throttle` | 예정 | [상세](docs/modules/throttle.md) |
...
위 코드는 목표로 하는 API 형태이며 각 모듈이 구현되기 전까지는 컴파일되지 않습니다.
...
Android SDK 37과 JDK 17 이상을 설치한 뒤 실행합니다.
